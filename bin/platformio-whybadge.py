...
def flasher_manifest(parts, ver):
    return {
        "name": "Meshtastic for WHY2025 badge (carrier board)",
        "version": ver,
        "builds": [
            {
                "chipFamily": "ESP32-C6",
                "improv": False,
                "parts": [
                    {"offset": offset, "path": Path(path).name}
...
def esp32_export_segments(source, target, env):
    sections = env.subst(env.get("FLASH_EXTRA_IMAGES"))
    firmware_name = env.subst("$BUILD_DIR/${PROGNAME}.bin")
    build_dir = Path(firmware_name).parent
    files_with_offsets = [(app_offset, firmware_name)]
    for section in sections:
        sect_adr, sect_file = section.split(" ", 1)
        file_path = Path(sect_file)
        if file_path.parent != build_dir:
            print(f"Copying {file_path} to {build_dir}")
            file_path = shutil.copy(file_path, build_dir)

        files_with_offsets.append((int(sect_adr, 16), str(file_path)))

    with open(env.subst("$BUILD_DIR/sections.json"), "w") as manifest_file:
        json.dump(
            flasher_manifest(files_with_offsets, verObj["long"]),
            manifest_file,
            indent=2,
        )
...
if platform.name == "espressif32":
    ...
    env.AddPostAction("$BUILD_DIR/${PROGNAME}.bin", esp32_export_segments)
